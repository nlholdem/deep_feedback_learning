...
void testXOR_ICO() {
	int nHidden[] = {2};
	int cat;
	DeepFeedbackLearning* deep_fbl = new DeepFeedbackLearning(3,nHidden,1,1);
	deep_fbl->initWeights(0.1);
	
	FILE* f=fopen("test_deep_fbl_cpp_learning_xor.dat","wt");
	...
	for (int e=0; e<epoch; e++) {
		cat = (int)((double)4*(((double)random())/((double)RAND_MAX)));
		...
		for(int n = 0; n < 1000;n++) {
			...
			input = inputs[cat];
			fprintf(f,"*%f *%f %f ",input[0], input[1],err);
			error[0] = err;
